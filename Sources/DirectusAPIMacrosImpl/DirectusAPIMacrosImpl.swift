...
struct DirectusClassRegistration: MemberMacro {
    static func expansion(
        ...
        let typeAlias = DeclSyntax(stringLiteral:
            """
            
            // Generated by @DirectusClassRegistration macro
            typealias DirectusDataCollection = DirectusAPI.DirectusData & DirectusAPI.DirectusCollection
            """
        )
...
struct DirectusAddToCollectionList: DeclarationMacro {
    static func expansion(
...
//        }
        ...
        for argument in argumentList {
            if let expr = argument.expression.as(MemberAccessExprSyntax.self) {
                let className = expr.base?.description.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
                assignments.append("\(className)?._register()")
            } else if let expr = argument.expression.as(DeclReferenceExprSyntax.self) {
                let className = expr.baseName.text
                assignments.append("\(className)._register()")
            }
        }
...
//        for argument in argumentList {
//            if let expr = argument.expression.as(MemberAccessExprSyntax.self) {
//                let className = expr.base?.description.trimmingCharacters(
